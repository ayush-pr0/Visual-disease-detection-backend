...
# path= "D:\\Work\\Disease Detection\\Dataset\\Dataset_BUSI_with_GT\\malignant\\malignant (1).png"
model = keras.models.load_model(os.path.join(os.getcwd(), "model\\breast_cancer.h5"))
resize = 150
class_names = ['Very Miled', 'Mailgnant', 'Normal']
...
def prediction(img_path):
    ...
    heatmap = make_gradcam_heatmap(image, model, last_conv_layer_name="block5_conv3")
    # plt.title("Heat Map")
    # plt.imshow(heatmap)
    # plt.show()
    cam_path = os.path.join(os.getcwd(), "result\\breast_cancer", "result_" + os.path.basename(img_path))
